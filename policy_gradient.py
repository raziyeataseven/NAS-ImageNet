...
class PolicyGradient:
    def __init__(self, config, train_set, test_set, use_cuda=False, in_channels=1):
    ...
    def play_episode(self):
        ...
        torch.save(net.state_dict(), './models_weights/' + str(acc).replace('.', '_') + '.pt')
        # compute the reward
        reward = acc

        # log action space with reward
        log_str = "Action Space:  "+ str(action) + ", last_reward:  "+str(reward)+"\n"
        log = open("log.txt", "a+")
        log.write(log_str)
        log.close()
